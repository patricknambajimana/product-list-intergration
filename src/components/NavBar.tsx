...
const Navbar: React.FC = () => {
  ...
  const firstCart = carts && carts.length > 0 ? carts[0] : null;
  const totalCartItems = firstCart ? firstCart.products.length : 0;
  return (
    <nav className="bg-green-900 text-white fixed top-0 left-0 right-0 z-20 shadow-md p-4">
      <div className="container mx-auto px-4 py-3 flex flex-col md:flex-row items-center justify-between gap-2">
        ...
        <div
          className={`flex-col md:flex md:flex-row md:items-center md:space-x-6 w-full md:w-auto absolute md:static left-0 right-0 top-14 bg-green-100 md:bg-transparent shadow-md md:shadow-none transition-all duration-300 ease-in-out ${
            isOpen ? "flex" : "hidden"
          }`}>
          {links.map((link) => (
            <NavLink
            ...
            <div className="relative">
              ...
              {profileOpen && (
                <div className="absolute right-0 mt-2 w-40 bg-white text-black rounded shadow-md p-2">
                  {user ? (
                    <>
                      <p className="px-3 py-1 text-sm border-b">
                        Hello,{" "}
                        <span className="font-bold">{user.username}</span>
                      </p>
                      <button
                        onClick={handleLogout}
                        className="w-full flex items-center gap-2 px-3 py-2 text-red-600 hover:bg-gray-100 rounded">
